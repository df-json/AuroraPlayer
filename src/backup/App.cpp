...
bool App::initialize()
{
    ...
    window_ = SDL_CreateWindow("Aurora Player", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
    if (!window_)
    {
        std::cerr << SDL_GetError() << '\n';
        return false;
    ...
    io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;
    {
        ...
        if (!fontPath.empty())
            io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 19.0f, &cfg);
        else
