...
bool MusicLibrary::addFolder(const std::string&p){std::error_code ec;if(!fs::is_directory(p,ec))return false;return db_.addLibraryFolder(fs::weakly_canonical(p,ec).string());}bool MusicLibrary::removeFolder(const std::string&p){bool a=db_.removeLibraryFolder(p);bool b=db_.removeSongsUnderFolder(p);return a&&b;}void MusicLibrary::wait(){if(worker_.joinable())worker_.join();}
