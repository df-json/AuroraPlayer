...
static void truncateText(const std::string &s) { ImGui::TextUnformatted(s.c_str()); }
...
SDL_Texture *UI::textureFor(const std::string &path)
{
    ...
    SDL_GetWindowData(window_, "AuroraRenderer"); /* renderer is stored by SDL window user data in App below when available */
    r = (SDL_Renderer *)SDL_GetWindowData(window_, "renderer");
    if (!r)
    {
...
void UI::renderLibrary()
{
    ...
    const char *tabs[] = {"Songs", "Albums", "Artists"};
    for (int i = 0; i < 3; i++)
    {
        if (i)
            ImGui::SameLine();
        if (ImGui::Selectable(tabs[i], libraryTab_ == i, ImGuiSelectableFlags_None, ImVec2(100, 30)))
            libraryTab_ = i;
    }
    ImGui::Separator();
    if (libraryTab_ == 0)
    {
        auto v = db_.songs();
        const char *sorts[] = {"Name", "Artist", "Album", "Date Added", "Play Count"};
        ImGui::SetNextItemWidth(150);
        ImGui::Combo("Sort", &sortMode_, sorts, 5);
        std::sort(v.begin(), v.end(), [&](const Song &a, const Song &b)
                  {if(sortMode_==1)return a.artist<b.artist;if(sortMode_==2)return a.album<b.album;if(sortMode_==3)return a.dateAdded>b.dateAdded;if(sortMode_==4)return a.playCount>b.playCount;return a.title<b.title; });
    ...
    {
        for (auto &a : db_.albums())
        {
            ImGui::PushID(a.title.c_str());
            if (ImGui::Selectable((a.title + "##album").c_str()))
            {
                selectedAlbumTitle_ = a.title;
                selectedAlbumArtist_ = a.artist;
            }
            ImGui::SameLine(350);
            ImGui::TextDisabled("%s • %d • %zu tracks", a.artist.c_str(), a.year, a.songs.size());
            ImGui::PopID();
        }
        if (!selectedAlbumTitle_.empty())
        {
            auto a = db_.album(selectedAlbumTitle_, selectedAlbumArtist_);
            if (a)
                renderAlbum(*a);
        }
    }
    ...
    {
        for (auto &a : db_.artists())
        {
            if (ImGui::Selectable((a.name + "##artist").c_str()))
                selectedArtistName_ = a.name;
            ImGui::SameLine(350);
            ImGui::TextDisabled("%zu songs", a.songs.size());
        }
        if (!selectedArtistName_.empty())
        {
            auto a = db_.artist(selectedArtistName_);
            if (a)
                renderArtist(*a);
        }
    }
...
void UI::renderAlbum(const Album &a)
{
    ...
    {
        ImGui::Image((ImTextureID)(intptr_t)t, ImVec2(110, 110));
        ImGui::SameLine();
    }
    ImGui::Text("Album: %s", a.title.c_str());
    ImGui::TextDisabled("%s • %d • %zu tracks", a.artist.c_str(), a.year, a.songs.size());
...
void UI::renderPlayerBar()
{
    ...
    {
        ImGui::Image((ImTextureID)(intptr_t)t, ImVec2(52, 52));
        ImGui::SameLine();
    }
    ImGui::BeginGroup();
    ...
    if (ImGui::Button(">|"))
        advance(true);
    ImGui::SameLine();
    if (ImGui::Button(shuffle_ ? "Shuffle ON" : "Shuffle"))
    {
        shuffle_ = !shuffle_;
        if (shuffle_)
    ...
    ImGui::SameLine();
    const char *rep[] = {"Repeat Off", "Repeat All", "Repeat One"};
    if (ImGui::Button(rep[repeatMode_]))
        repeatMode_ = (repeatMode_ + 1) % 3;
    ImGui::SameLine();
    float pos = (float)player_.position(), dur = (float)player_.duration();
    ImGui::SetNextItemWidth(300);
    if (dur > 0 && ImGui::SliderFloat("##progress", &pos, 0, dur, ""))
        player_.seek(pos);
    ImGui::SameLine();
    ImGui::Text("%s / %s", timeFmt(pos).c_str(), timeFmt(dur).c_str());
    ImGui::SameLine();
    float v = player_.volume();
    ImGui::SetNextItemWidth(110);
    if (ImGui::SliderFloat("##vol", &v, 0, 1, "Volume %.0f%%"))
        player_.setVolume(v);
    ImGui::SameLine();
...
void UI::render()
{
    ...
    renderSidebar();
    ImGui::SameLine();
    ImGui::BeginChild("content", ImVec2(0, -100), false);
    switch (page_)
    {
    case 0:
        renderHome();
